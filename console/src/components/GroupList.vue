  ...
  try {
    const promises = draggableGroups.value?.map((group: LinkGroup, index) => {
      ...
      return apiClient.put(
        `/apis/cn.cszcyl.products/v1alpha1/productgroups/${group.metadata.name}`,
        group
      );
    });
    if (promises) {
...
const handleDelete = async (group: LinkGroup) => {
  Dialog.warning({
    title: "确定要删除该分组吗？",
    description: "将同时删除该分组下的所有产品，该操作不可恢复。",
    confirmType: "danger",
    onConfirm: async () => {
      try {
        ...
        const { data } = await apiClient.get<LinkList>(
          `/apis/api.plugin.halo.run/v1alpha1/plugins/cs-zcyl-products/links`,
          {
            params: {
              page: 0,
              size: 0,
