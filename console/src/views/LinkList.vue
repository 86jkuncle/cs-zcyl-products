  ...
  try {
    const promises = draggableLinks.value?.map((link: Link, index) => {
      ...
      return apiClient.put(
        `/apis/cn.cszcyl.products/v1alpha1/products/${link.metadata.name}`,
        link
      );
    });
    if (promises) {
    ...
    onConfirm: async () => {
      try {
        const promises = selectedLinks.value.map((link) => {
          return apiClient.delete(`/apis/cn.cszcyl.products/v1alpha1/products/${link}`);
        });
        if (promises) {
          await Promise.all(promises);
        }
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      const promises = parsed.map((link) => {
        return apiClient.post("/apis/cn.cszcyl.products/v1alpha1/products", link);
      });
      if (promises) {
        await Promise.all(promises);
      }
    } else {
      await apiClient.post("/apis/cn.cszcyl.products/v1alpha1/products", parsed);
    }
  } catch (e) {
    console.error(e);
  } finally {
      ...
      <div class="links-flex-1">
        <VCard :body-class="['!p-0']">
          ...
          <Transition v-else-if="!links?.length" appear name="fade">
            <VEmpty message="你可以新建产品" title="当前没有产品">
              <template #actions>
                <VSpace>
              ...
              <template #item="{ element: link }">
                <li>
                  ...
                  >
                    ...
                    <template #start>
                      <VEntityField :title="link.spec.name" />
                      <VEntityField>
                        <template #description>
                          {{ "/link-product/" + link.metadata.name }}
                        </template>
                      </VEntityField>
                      <!-- <VEntityField>
                        <template #description>
